  ...
  if (compact) {
    return (
      <div className="space-y-3">
        ...
        <div className={`grid ${FIREBASE_PRESETS.length === 1 ? 'grid-cols-1' : 'grid-cols-2'} gap-2`}>
    ...
    <div className="bg-slate-50 border border-slate-200 rounded-2xl p-4 sm:p-5 space-y-4">
      ...
      {!isGestor && (
        <div className="p-3.5 bg-amber-500/10 border border-amber-500/30 rounded-xl text-xs text-amber-900 flex items-start space-x-2.5">
          ...
          <div>
            <p className="font-bold text-indigo-950">Banco de Dados Conectado: {FIREBASE_PRESETS.find(p => p.config.projectId === activeProjectId)?.name || activeProjectId}</p>
            <p className="text-indigo-900 text-[11px] mt-0.5 leading-relaxed">
              Toda a plataforma opera no banco ativo para todos os usuários e dispositivos em tempo real.
            </p>
          </div>
        </div>
      )}
