...
export default function GestorDashboard({
  ...
  const normalizeMapCode = (m?: string) => m ? String(m).replace(/^0+/, '').trim() : '';
  ...
  const handleDeleteMapsByDate = () => {
    ...
    requestConfirm(
      ...
      `Tem certeza que deseja excluir permanentemente todos os mapas importados, descarregamentos e auditorias na data ${new Date(selectedDeleteDate + 'T00:00:00').toLocaleDateString('pt-BR')}? Esta ação removerá as rotas, auditorias, descarregamentos e vales associados de forma definitiva.`,
      () => {
        const routesToDelete = importedRoutes.filter(r => r.routeDate === selectedDeleteDate);
        const mapCodesToDelete = new Set(routesToDelete.map(r => (r.routeMap || '').toUpperCase().trim()));
        const normMapsToDelete = new Set(routesToDelete.map(r => normalizeMapCode(r.routeMap).toUpperCase()));
        const platesToDelete = new Set(routesToDelete.map(r => (r.plate || '').toUpperCase().trim()).filter(Boolean));

        // Also include map codes from audits or carregamentos for this date
        audits.forEach(a => {
          if (a.arrivalDate === selectedDeleteDate) {
            if (a.routeMap) {
              mapCodesToDelete.add(a.routeMap.toUpperCase().trim());
              normMapsToDelete.add(normalizeMapCode(a.routeMap).toUpperCase());
            }
        ...
        carregamentos.forEach(c => {
          if (c.routeDate === selectedDeleteDate || c.createdAt?.startsWith(selectedDeleteDate)) {
            if (c.routeMap) {
              mapCodesToDelete.add(c.routeMap.toUpperCase().trim());
              normMapsToDelete.add(normalizeMapCode(c.routeMap).toUpperCase());
            }
        ...
        const updatedRoutes = importedRoutes.filter(r => {
          ...
          return r.routeDate !== selectedDeleteDate && !normMapsToDelete.has(rMapNorm) && !mapCodesToDelete.has(rMapUpper);
        });
        ...
        const updatedAudits = audits.filter(a => {
          ...
          const isDateMatch = a.arrivalDate === selectedDeleteDate;
          const isMapMatch = normMapsToDelete.has(aMapNorm) || mapCodesToDelete.has(aMapUpper);
        ...
        if (onSaveCarregamentos) {
          updatedCarreg = carregamentos.filter(c => {
            ...
            const isDateMatch = c.routeDate === selectedDeleteDate || c.createdAt?.startsWith(selectedDeleteDate);
            const isMapMatch = normMapsToDelete.has(cMapNorm) || mapCodesToDelete.has(cMapUpper);
        ...
        // 4. Update vales
        const updatedVales = vales.filter(v => {
          const vMapNorm = normalizeMapCode(v.routeMap).toUpperCase();
          const vMapUpper = (v.routeMap || '').toUpperCase().trim();
          return !normMapsToDelete.has(vMapNorm) && !mapCodesToDelete.has(vMapUpper);
        });
        onSaveVales(updatedVales);

        // 5. Direct Firestore sync
        if (isClientFirebaseActive()) {
    ...
    <div className="w-full px-2 sm:px-6 lg:px-8 py-4 sm:py-8" id="gestor_view">
      ...
      {gestorTab === 'dashboard' && (
        ...
        <div className="space-y-8" id="gestor_dashboard">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-xs space-y-5" id="gestor_histograma_anexo">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4 pb-3 border-b border-slate-100">
              ...
              <div className="flex items-center space-x-2">
                <button
                  onClick={() => setGestorTab('efd_histograma')}
                  className="px-3.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-xs font-bold transition flex items-center space-x-1.5 shadow-xs cursor-pointer"
                >
                  <Sparkles className="h-3.5 w-3.5" />
                  <span>Auditar Todos os {EFD_REAL_RECORDS_160.length} Registros</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div 
                onClick={() => setGestorTab('efd_histograma')}
                className="p-3 rounded-xl border bg-emerald-50/40 border-emerald-200 hover:bg-emerald-50 transition cursor-pointer flex items-center justify-between"
              >
                <div className="flex items-center space-x-2">
                  <Award className="h-4 w-4 text-emerald-600" />
                  <div>
                    <span className="font-bold text-slate-800 text-xs block">Paulo Pereira: 100.0% EFD</span>
                    <span className="text-[10px] text-slate-500">Pátio • 100% Meta</span>
                  </div>
                </div>
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <span className="text-[10px] text-slate-500">≥ 14h • 100% Meta</span>
                  </div>
                </div>
                <span className="text-[10px] font-bold text-emerald-700 bg-white px-2 py-0.5 rounded border border-emerald-200">100% Meta</span>
              </div>
            ...
            {/* Cards de Ciclo D0 a D4 (Dias que os veículos passaram em rota: Importação ao Fechamento) */}
            {(() => {
              const totalEfd = EFD_REAL_RECORDS_160.length;
              const d0Total = EFD_REAL_RECORDS_160.filter(r => r.cycle === 'D0').length;
              const d1Total = EFD_REAL_RECORDS_160.filter(r => r.cycle === 'D1').length;
              const d2Total = EFD_REAL_RECORDS_160.filter(r => r.cycle === 'D2').length;
              const d3Total = EFD_REAL_RECORDS_160.filter(r => r.cycle === 'D3').length;
              const d4Total = EFD_REAL_RECORDS_160.filter(r => r.cycle === 'D4').length;

              const d0Pct = totalEfd > 0 ? (d0Total / totalEfd) * 100 : 0;
              const d1Pct = totalEfd > 0 ? (d1Total / totalEfd) * 100 : 0;
              const d2Pct = totalEfd > 0 ? (d2Total / totalEfd) * 100 : 0;
              const d3Pct = totalEfd > 0 ? (d3Total / totalEfd) * 100 : 0;
              const d4Pct = totalEfd > 0 ? (d4Total / totalEfd) * 100 : 0;
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
                  ...
                  <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[10px] font-bold text-sky-700 bg-sky-100 px-1.5 py-0.5 rounded font-mono">
                        0 dias em rota
                      </span>
                    </div>
                    <p className="text-[9px] text-slate-500 mt-0.5 font-medium">Data de importação = fechamento (Meta: 65% • {d0Pct.toFixed(1)}% frota)</p>
                    <div className="mt-2 flex items-baseline justify-between">
                      <span className="text-xl font-extrabold text-slate-900">{d0Total} <span className="text-[10px] font-normal text-slate-400">veículos</span></span>
                      <span className="text-xs font-extrabold text-sky-700">100.0% EFD</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
                      <div className="bg-sky-500 h-full rounded-full" style={{ width: `${d0Pct}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                    ...
                    <p className="text-[9px] text-slate-500 mt-0.5 font-medium">Fechamento 1 dia após importação / pernoite (Meta: 32% • {d1Pct.toFixed(1)}% frota)</p>
                    <div className="mt-2 flex items-baseline justify-between">
                      <span className="text-xl font-extrabold text-slate-900">{d1Total} <span className="text-[10px] font-normal text-slate-400">veículos</span></span>
                      <span className="text-xs font-extrabold text-emerald-700">100.0% EFD</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
                      <div className="bg-emerald-500 h-full rounded-full" style={{ width: `${d1Pct}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                    ...
                    <p className="text-[9px] text-slate-500 mt-0.5 font-medium">Fechamento 2 dias após importação (Meta: 2% • {d2Pct.toFixed(1)}% frota)</p>
                    <div className="mt-2 flex items-baseline justify-between">
                      <span className="text-xl font-extrabold text-slate-900">{d2Total} <span className="text-[10px] font-normal text-slate-400">veículos</span></span>
                      <span className="text-xs font-extrabold text-blue-700">100.0% EFD</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
                      <div className="bg-blue-500 h-full rounded-full" style={{ width: `${d2Pct}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[10px] font-bold text-amber-700 bg-amber-100 px-1.5 py-0.5 rounded font-mono">
                        3 dias em rota
                      </span>
                    </div>
                    <p className="text-[9px] text-slate-500 mt-0.5 font-medium">Fechamento 3 dias após importação (Meta: 1% • {d3Pct.toFixed(1)}% frota)</p>
                    <div className="mt-2 flex items-baseline justify-between">
                      <span className="text-xl font-extrabold text-slate-900">{d3Total} <span className="text-[10px] font-normal text-slate-400">veículos</span></span>
                      <span className="text-xs font-extrabold text-amber-700">100.0% EFD</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
                      <div className="bg-amber-500 h-full rounded-full" style={{ width: `${d3Pct}%` }} />
                    </div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-[10px] font-bold text-slate-600 bg-slate-200 px-1.5 py-0.5 rounded font-mono">
                        4+ dias em rota
                      </span>
                    </div>
                    <p className="text-[9px] text-slate-500 mt-0.5 font-medium">Fechamento 4+ dias após importação (Sem registros)</p>
                    <div className="mt-2 flex items-baseline justify-between">
                      <span className="text-xl font-extrabold text-slate-900">{d4Total} <span className="text-[10px] font-normal text-slate-400">veículos</span></span>
                      <span className="text-xs font-extrabold text-emerald-700">100.0% EFD</span>
                    </div>
                    <div className="w-full bg-slate-200 h-1.5 rounded-full mt-2 overflow-hidden">
