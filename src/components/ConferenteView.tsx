...
export default function ConferenteView({
  ...
  const autoSelectDriverForRoute = (routeInfo: { driverId?: string; driverName?: string } | ImportedRoute | null | undefined) => {
    if (!routeInfo) {
      setDriverId('');
      setTempDriverName('');
      return;
    ...
    const dId = (routeInfo as any).driverId;
    const dName = (routeInfo as any).driverName;

    // 1. Try finding by driverId first
    if (dId && dId !== 'solicitar_cadastro') {
      const matchedById = findRegisteredDriver(dId);
      if (matchedById) {
        setDriverId(matchedById.id);
        setTempDriverName('');
        return;
      }
    }

    // 2. Try finding by driverName
    if (dName && dName !== 'Não informado' && dName !== 'Motorista Não Informado') {
      const matchedByName = findRegisteredDriver(dName);
      if (matchedByName) {
        setDriverId(matchedByName.id);
        setTempDriverName('');
        return;
      }
    }
    ...
    // 3. Fallback
    const fallbackVal = dName || dId;
    if (fallbackVal && fallbackVal !== 'Não informado') {
      setDriverId('solicitar_cadastro');
      setTempDriverName(fallbackVal.toUpperCase());
    } else {
      setDriverId('');
      setTempDriverName('');
    }
  ...
  const findMapInfo = (mCode: string) => {
    ...
    if (openItem) {
      return { plate: openItem.plate, driverName: openItem.driverName, driverId: (openItem as any).driverId };
    }
    const impItem = importedRoutes.find(x => x.routeMap.toUpperCase() === mapUpper);
    if (impItem) {
      return { 
        plate: impItem.plate, 
        driverName: (impItem as any).driverName || (impItem.driverId ? getDriverName(impItem.driverId) : undefined),
        driverId: impItem.driverId 
      };
    }
    const audItem = audits.find(a => a.routeMap.toUpperCase() === mapUpper);
    if (audItem) {
      return { plate: audItem.plate, driverName: getDriverName(audItem.driverId), driverId: audItem.driverId };
    }
    return undefined;
  };
  ...
  const handleAddMapTag = (value: string) => {
    ...
    setSelectedRouteMaps(prev => {
      ...
      if (firstInfo) {
        autoSelectDriverForRoute({ driverId: firstInfo.driverId, driverName: firstInfo.driverName });
      }
  ...
  const openRoutesList = useMemo(() => {
    ...
    (importedRoutes || []).forEach(route => {
      ...
      if ((!isClosed && !isSubmittedToFiscal && (route.status as string) !== 'em_analise') || isReopeningReq) {
        ...
        const unTime = route.unloadingEndTime || matchingAudit?.unloadingEndTime || route.unloadingStartTime || matchingAudit?.unloadingStartTime;
        ...
        list.push({
          ...
          audit: matchingAudit,
          descarregamentoStatus: isPernoite ? 'PERNOITE' : descStatus,
          isPernoite,
          dock,
          empilhadorName: empName,
          unloadingTime: unTime
        });
      }
    });
    ...
    (audits || []).forEach(audit => {
      ...
      if ((isReopeningReq || isAuditActive) && !processedMaps.has(auditMapUpper) && !processedMaps.has(auditMapNorm)) {
        processedMaps.add(auditMapUpper);
        processedMaps.add(auditMapNorm);
        ...
        const unTime = audit.unloadingEndTime || matchingRoute?.unloadingEndTime || audit.unloadingStartTime || matchingRoute?.unloadingStartTime;
  ...
  const mapsToday = openRoutesList.length;
                    ...
                    <div className="bg-amber-50 border border-amber-200 p-3 rounded-lg flex flex-col sm:flex-row sm:items-center justify-between gap-2 text-xs">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setPlate(uniquePlates.join(' / '));
                          if (firstRoute) {
                            autoSelectDriverForRoute({ driverId: firstRoute.driverId, driverName: firstRoute.driverName });
                          }
                        }}
